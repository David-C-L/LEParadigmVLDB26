...
#define CURL_MAX_WRITE_SIZE (8*1024*1024) // 16384
#endif
...
/*
 * NAME curl_slist_append_nodup()
 *
 * DESCRIPTION
 *
 * Appends a string to a linked list. If no list exists, it will be created
 * first. Returns the new list, after appending. Creates no duplicate so
 * data lifetime must exceed that of the list.
 */
CURL_EXTERN struct curl_slist *curl_slist_append_nodup(struct curl_slist *list,
                                                       char *data);
