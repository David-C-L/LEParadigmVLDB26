...
// Helper function for creating span expressions (used in PartialTable benchmarks)
...
auto createSpansInt = [](auto... values) {
  ...
  auto s1 = boss::expressions::atoms::Span<intType>(std::move(v1));
  SpanArguments args;
...
// PartialTable benchmark functions
void runPartialTableBasic(benchmark::State& state) {
  auto eval = [](auto&& expression) {
    boss::expressions::ExpressionSpanArguments spans;
    spans.emplace_back(boss::expressions::Span<std::string>(librariesToTest()));
    return boss::evaluate("EvaluateInEngines"_(
  ...
  for(auto _ : state) { // NOLINT
    ...
    auto partialTable = "PartialTable"_(
      "Table"_(
        "key"_(createSpansInt(10, 20, 30, 40, 50, 60)),
        "payload"_(createSpansInt(100, 200, 300, 400, 500, 600)),
        "row_index"_(createSpansInt(0, 2, 4))
      ),
      "row_index"_
    );
#else
    auto partialTable = "PartialTable"_(
      "Table"_(
        "Column"_("key"_, createSpansInt(10, 20, 30, 40, 50, 60)),
        "Column"_("payload"_, createSpansInt(100, 200, 300, 400, 500, 600)),
        "Column"_("row_index"_, createSpansInt(0, 2, 4))
      ),
      "row_index"_
    );
#endif

    vtune.startSampling("PartialTableBasic");
    state.ResumeTiming();
    
    auto result = eval("Project"_(std::move(partialTable), 
				  "As"_("key"_, "key"_, "payload"_, "payload"_, "row_index"_, "row_index"_)));
...
void runPartialTableWithSelect(benchmark::State& state) {
  auto eval = [](auto&& expression) {
    boss::expressions::ExpressionSpanArguments spans;
    spans.emplace_back(boss::expressions::Span<std::string>(librariesToTest()));
    return boss::evaluate("EvaluateInEngines"_(
  ...
  for(auto _ : state) { // NOLINT
    ...
    vtune.startSampling("PartialTableSelect");
    state.ResumeTiming();
    
    auto result = eval("Select"_(
				 "Project"_(std::move(partialTable), "As"_("key"_, "key"_, "payload"_, "payload"_, "row_index"_, "row_index"_)),
      "Where"_("Greater"_("key"_, 20))
    ));
    benchmark::DoNotOptimize(result);
...
void runPartialTableWithGroup(benchmark::State& state) {
  auto eval = [](auto&& expression) {
    boss::expressions::ExpressionSpanArguments spans;
    spans.emplace_back(boss::expressions::Span<std::string>(librariesToTest()));
    return boss::evaluate("EvaluateInEngines"_(
  ...
  for(auto _ : state) { // NOLINT
    ...
    auto partialTable = "PartialTable"_(
      "Table"_(
        "category"_(createSpansInt(1, 2, 1, 2, 1, 2)),
        "value"_(createSpansInt(10, 20, 30, 40, 50, 60)),
        "row_index"_(createSpansInt(0, 2, 4))
      ),
      "row_index"_
    );
    ...
    auto partialTable = "PartialTable"_(
      "Table"_(
        "Column"_("category"_, createSpansInt(1, 2, 1, 2, 1, 2)),
        "Column"_("value"_, createSpansInt(10, 20, 30, 40, 50, 60)),
        "Column"_("row_index"_, createSpansInt(0, 2, 4))
      ),
      "row_index"_
    );
    ...
    vtune.startSampling("PartialTableGroup");
    state.ResumeTiming();
    
    auto result = eval("Group"_(
      "Project"_(std::move(partialTable), "As"_("category"_, "category"_, "value"_, "value"_)),
      "category"_, 
      "As"_("sum_value"_, "Sum"_("value"_))
    ));
    benchmark::DoNotOptimize(result);
...
void runPartialTableWithOrder(benchmark::State& state) {
  auto eval = [](auto&& expression) {
    boss::expressions::ExpressionSpanArguments spans;
    spans.emplace_back(boss::expressions::Span<std::string>(librariesToTest()));
    return boss::evaluate("EvaluateInEngines"_(
  ...
  for(auto _ : state) { // NOLINT
    ...
    auto partialTable = "PartialTable"_(
      "Table"_(
        "key"_(createSpansInt(30, 10, 20, 60, 40, 50)),
        "payload"_(createSpansInt(300, 100, 200, 600, 400, 500)),
        "row_index"_(createSpansInt(1, 2, 4))
      ),
      "row_index"_
    );
#else
    auto partialTable = "PartialTable"_(
      "Table"_(
        "Column"_("key"_, createSpansInt(30, 10, 20, 60, 40, 50)),
        "Column"_("payload"_, createSpansInt(300, 100, 200, 600, 400, 500)),
        "Column"_("row_index"_, createSpansInt(1, 2, 4))
      ),
      "row_index"_
    );
#endif

    vtune.startSampling("PartialTableOrder");
    state.ResumeTiming();
    
    auto result = eval("Order"_(
      "Project"_(std::move(partialTable), "As"_("key"_, "key"_, "payload"_, "payload"_)),
      "By"_("key"_)
    ));
    benchmark::DoNotOptimize(result);
...
void runPartialTableWithJoin(benchmark::State& state) {
  auto eval = [](auto&& expression) {
    boss::expressions::ExpressionSpanArguments spans;
    spans.emplace_back(boss::expressions::Span<std::string>(librariesToTest()));
    return boss::evaluate("EvaluateInEngines"_(
  ...
  for(auto _ : state) { // NOLINT
...
#ifdef USE_NEW_TABLE_FORMAT
    // Create first partial table (customers)
    auto partialTableCustomers = "PartialTable"_(
      "Table"_(
        "c_cust_id"_(createSpansInt(1, 2, 3, 4, 5, 6)),
        "c_cust_name"_(createSpansInt(101, 102, 103, 104, 105, 106)),
        "c_cust_index"_(createSpansInt(0, 2, 4))
      ),
      "c_cust_index"_
    );

    // Create second partial table (orders)
    auto partialTableOrders = "PartialTable"_(
      "Table"_(
        "o_order_id"_(createSpansInt(10, 20, 30, 40, 50, 60)),
        "o_cust_id"_(createSpansInt(1, 3, 2, 1, 4, 5)),
        "o_amount"_(createSpansInt(100, 300, 200, 150, 400, 500)),
        "o_order_index"_(createSpansInt(0, 3, 5))
      ),
      "o_order_index"_
    );
#else
    ...
    auto partialTableCustomers = "PartialTable"_(
      "Table"_(
        "Column"_("c_cust_id"_, createSpansInt(1, 2, 3, 4, 5, 6)),
        "Column"_("c_cust_name"_, createSpansInt(101, 102, 103, 104, 105, 106)),
        "Column"_("c_cust_index"_, createSpansInt(0, 2, 4))
      ),
      "c_cust_index"_
    );
    ...
    auto partialTableOrders = "PartialTable"_(
      "Table"_(
        "Column"_("o_order_id"_, createSpansInt(10, 20, 30, 40, 50, 60)),
        "Column"_("o_cust_id"_, createSpansInt(1, 3, 2, 1, 4, 5)),
        "Column"_("o_amount"_, createSpansInt(100, 300, 200, 150, 400, 500)),
        "Column"_("o_order_index"_, createSpansInt(0, 3, 5))
      ),
      "o_order_index"_
    );
#endif

    vtune.startSampling("PartialTableJoin");
    state.ResumeTiming();
    
    auto result = eval("Join"_(
      "Project"_(std::move(partialTableCustomers), 
                 "As"_("c_cust_id"_, "c_cust_id"_, "c_cust_name"_, "c_cust_name"_, "c_cust_index"_, "c_cust_index"_)),
      "Project"_(std::move(partialTableOrders), 
                 "As"_("o_order_id"_, "o_order_id"_, "o_cust_id"_, "o_cust_id"_, "o_amount"_, "o_amount"_, "o_order_index"_, "o_order_index"_)),
      "Where"_("Equal"_("c_cust_id"_, "o_cust_id"_))
    ));
    benchmark::DoNotOptimize(result);
...
void runPartialTableWithDifferentIndexName(benchmark::State& state) {
  auto eval = [](auto&& expression) {
    boss::expressions::ExpressionSpanArguments spans;
    spans.emplace_back(boss::expressions::Span<std::string>(librariesToTest()));
    return boss::evaluate("EvaluateInEngines"_(
  ...
  for(auto _ : state) { // NOLINT
    ...
    auto partialTable = "PartialTable"_(
      "Table"_(
        "name"_(createSpansInt(100, 200, 300, 400, 500)),
        "score"_(createSpansInt(95, 87, 92, 78, 88)),
        "valid_ids"_(createSpansInt(0, 3, 4))
      ),
      "valid_ids"_
    );
#else
    auto partialTable = "PartialTable"_(
      "Table"_(
        "Column"_("name"_, createSpansInt(100, 200, 300, 400, 500)),
        "Column"_("score"_, createSpansInt(95, 87, 92, 78, 88)),
        "Column"_("valid_ids"_, createSpansInt(0, 3, 4))
      ),
      "valid_ids"_
    );
#endif

    vtune.startSampling("PartialTableDifferentIndex");
    state.ResumeTiming();
    
    auto result = eval("Project"_(std::move(partialTable), 
                                 "As"_("name"_, "name"_, "score"_, "score"_)));
    benchmark::DoNotOptimize(result);
...
void runPartialTableErrorHandling(benchmark::State& state) {
  auto eval = [](auto&& expression) {
    boss::expressions::ExpressionSpanArguments spans;
    spans.emplace_back(boss::expressions::Span<std::string>(librariesToTest()));
    return boss::evaluate("EvaluateInEngines"_(
  ...
  auto error_handling = [](auto&& result) {
    // For error handling benchmark, we expect errors
    if(!holds_alternative<boss::ComplexExpression>(result)) {
  ...
  for(auto _ : state) { // NOLINT
    ...
    auto partialTable = "PartialTable"_(
      "Table"_(
        "key"_(createSpansInt(10, 20, 30)),
        "payload"_(createSpansInt(100, 200, 300))
      ),
      "missing_column"_ // This column doesn't exist
    );
#else
    auto partialTable = "PartialTable"_(
      "Table"_(
        "Column"_("key"_, createSpansInt(10, 20, 30)),
        "Column"_("payload"_, createSpansInt(100, 200, 300))
      ),
      "missing_column"_
    );
#endif

    vtune.startSampling("PartialTableErrorHandling");
    state.ResumeTiming();
    
    // This should throw an exception since the index column doesn't exist
    try {
      auto result = eval("Project"_(std::move(partialTable), "As"_("key"_, "key"_)));
      benchmark::DoNotOptimize(result);
      // If we reach here, it means no exception was thrown, which is unexpected
      failed = true;
    } catch(...) {
      // Exception occurred as expected for error handling test
    }
...
void runPartialTablePerformanceOptimization(benchmark::State& state) {
  auto eval = [](auto&& expression) {
    boss::expressions::ExpressionSpanArguments spans;
    spans.emplace_back(boss::expressions::Span<std::string>(librariesToTest()));
    return boss::evaluate("EvaluateInEngines"_(
  ...
  // Create large dataset with 60K elements to trigger optimization
  std::vector<intType> largeKeys, largePayloads, largeIndices;
  for(int i = 0; i < 60000000; i++) {
    largeKeys.push_back(i * 10);
    largePayloads.push_back(i * 100);
    if(i % 10 == 0) {  // Use every 10th index to create sparse dataset
      largeIndices.push_back(i);
    }
  }

  // Helper to create large spans
  auto createLargeSpanInt = [](const std::vector<intType>& values) {
    using SpanArguments = boss::expressions::ExpressionSpanArguments;
    auto s1 = boss::expressions::atoms::Span<intType>(std::vector<intType>(values));
    SpanArguments args;
    args.emplace_back(std::move(s1));
    return boss::expressions::ComplexExpression("List"_, {}, {}, std::move(args));
  };
  ...
  for(auto _ : state) { // NOLINT
    ...
    auto largePartialTable = "PartialTable"_(
      "Table"_(
        "key"_(createLargeSpanInt(largeKeys)),
        "payload"_(createLargeSpanInt(largePayloads)),
        "row_index"_(createLargeSpanInt(largeIndices))
      ),
      "row_index"_
    );

    vtune.startSampling("PartialTableLargeOptimization");
    state.ResumeTiming();
    
    auto result = eval("Project"_(std::move(largePartialTable), 
    ...
    if (printOnce) {
      std::cout << "LARGE PartialTable processed (60K elements, optimization should bypass dictionary vectors)" << std::endl;
      printOnce = false;
    }
...
template <typename Func>
void RegisterPartialTableBenchmark(std::string const& name, Func&& func) {
  RegisterBenchmarkNolint(name.c_str(), func)->Iterations(1);
}
...
void initAndRunBenchmarks(int argc, char** argv) {
  ...
  // Register PartialTable benchmarks
  RegisterPartialTableBenchmark("PartialTableBasic", runPartialTableBasic);
  RegisterPartialTableBenchmark("PartialTableWithSelect", runPartialTableWithSelect);
  RegisterPartialTableBenchmark("PartialTableWithGroup", runPartialTableWithGroup);
  RegisterPartialTableBenchmark("PartialTableWithOrder", runPartialTableWithOrder);
  RegisterPartialTableBenchmark("PartialTableWithJoin", runPartialTableWithJoin);
  RegisterPartialTableBenchmark("PartialTableWithDifferentIndexName", runPartialTableWithDifferentIndexName);
  RegisterPartialTableBenchmark("PartialTableErrorHandling", runPartialTableErrorHandling);
  RegisterPartialTableBenchmark("PartialTableBasicPerfOptLargeInput", runPartialTablePerformanceOptimization);
