...
template<typename T>
static ExpressionSpanArgument extractNonNullSpan(const std::shared_ptr<facebook::velox::BaseVector>& vec, 
                                                 const std::vector<size_t>& validIndices) {
  std::vector<T> validValues;
  validValues.reserve(validIndices.size());
  auto* data = vec->values()->as<T>();
  for (size_t idx : validIndices) {
    validValues.push_back(data[idx]);
  }
  auto* ptr = validValues.data();
  auto size = validValues.size();
  spanReferenceCounter.add(ptr, [v = std::move(validValues)]() {});
  return boss::Span<T>(ptr, size, [ptr]() { spanReferenceCounter.remove(ptr); });
}

static ExpressionSpanArgument veloxtoSpan(VeloxVectorPayload&& payload) {
  auto& vec = payload.vec;
  
  // // If this is a FlatVector with nulls, we need to extract only non-null values
  // if (vec->nulls()) {
  //   std::cout << "USING NULLS" << std::endl;
  //   // Handle vectors with null values by extracting only non-null elements
  //   std::vector<size_t> validIndices;
  //   validIndices.reserve(vec->size());
  ...
  //     return extractNonNullSpan<int8_t>(vec, validIndices);
  //   case TypeKind::SMALLINT:
  //     return extractNonNullSpan<int16_t>(vec, validIndices);
  //   case TypeKind::INTEGER:
  //     return extractNonNullSpan<int32_t>(vec, validIndices);
  //   case TypeKind::BIGINT:
  //     return extractNonNullSpan<int64_t>(vec, validIndices);
  //   case TypeKind::REAL:
  //     return extractNonNullSpan<float_t>(vec, validIndices);
  //   case TypeKind::DOUBLE:
  //     return extractNonNullSpan<double_t>(vec, validIndices);
  //   default:
  //     throw std::runtime_error("veloxToSpan: unsupported type with nulls: " +
  ...
  case TypeKind::TINYINT:
    return createBossSpan<int8_t>(std::move(payload));
  case TypeKind::SMALLINT:
    return createBossSpan<int16_t>(std::move(payload));
  case TypeKind::INTEGER:
...
template <typename T>
VectorPtr spanToVelox(boss::Span<T>&& span, memory::MemoryPool* pool, BufferPtr& indices, BufferPtr nullsBuffer = nullptr) {
  ...
  auto createVeloxVector = [&indices, &createDictVector, &pool](auto&& span, auto bossType) {
    ...
    auto flatVecPtr = importFromBossAsOwner(bossType, std::move(bossArray), pool, indices);
    return flatVecPtr;
    // if(indices == nullptr) {
    //   return flatVecPtr;
    // }
  ...
  } else if constexpr(std::is_same_v<T, int16_t> || std::is_same_v<T, int16_t const>) {
    return createVeloxVector(std::move(span), BossType::bSHORT);
...
getColumns(ComplexExpression&& expression, memory::MemoryPool* pool) {
  ...
  std::vector<BufferPtr> precomputedNullsBuffer;
  std::string partialTableIndexColumnName;
  ...
  // Using compacted vector
  if(expression.getHead().getName() == "PartialTable") {
    auto [head, statics, dynamics, spans] = std::move(expression).decompose();
    auto it = std::move_iterator(dynamics.begin());
  
    expression = std::get<ComplexExpression>(std::move(*it++));

    auto indexColumnName = std::get<boss::Symbol>(std::move(*it)).getName();
    std::transform(indexColumnName.begin(), indexColumnName.end(), indexColumnName.begin(), ::tolower);
    partialTableIndexColumnName = indexColumnName;
  
    auto [tHead, tStatics, tDynamics, tSpans] = std::move(expression).decompose();
    std::vector<BufferPtr> partialIndices;
  
    for(size_t colI = 0; colI < tDynamics.size(); colI++) {
      auto& column = tDynamics[colI];
#ifdef USE_NEW_TABLE_FORMAT
      auto columnName = get<ComplexExpression>(column).getHead().getName();
#else
      auto columnName = get<Symbol>(get<ComplexExpression>(column).getDynamicArguments().at(0)).getName();
#endif
      std::transform(columnName.begin(), columnName.end(), columnName.begin(), ::tolower);
    
      if(columnName == indexColumnName) {
	auto complexColumnCopy = get<ComplexExpression>(std::move(column));
	auto [cHead, cStatics, cDynamics, cSpans] = std::move(complexColumnCopy).decompose();
      
#ifdef USE_NEW_TABLE_FORMAT
	boss::Expression dynamic = std::move(cDynamics[0]);
#else
	boss::Expression dynamic = std::move(cDynamics[1]);
#endif
      
	auto list = transformDynamicsToSpans(std::move(dynamic));
	auto complexList = get<ComplexExpression>(std::move(list));
	auto [listHead, listUnused_, listDynamics, listSpans] = std::move(complexList).decompose();
      
	partialIndices = getIndices(std::move(listSpans));
#ifdef USE_NEW_TABLE_FORMAT
	cDynamics[0] = "List"_();
#else
        cDynamics[1] = "List"_();
	...
	column = boss::ComplexExpression{std::move(cHead), std::move(cStatics),
	  std::move(cDynamics), std::move(cSpans)};
	break;
      }
    }
  
    if(partialIndices.empty()) {
      throw std::runtime_error("PartialTable: Index column '" + indexColumnName + "' not found");
    }
    indicesVec = std::move(partialIndices);
    expression = boss::ComplexExpression{std::move(tHead), std::move(tStatics),
      std::move(tDynamics), std::move(tSpans)};
  }
  ...
  std::for_each(
	...
	auto list = transformDynamicsToSpans(std::move(dynamic));
        auto [listHead, listUnused_, listDynamics, listSpans] = std::move(list).decompose();
        if(listSpans.empty() && !(!partialTableIndexColumnName.empty() && columnName == partialTableIndexColumnName)) {
          return;
        }
        ...
        auto getVeloxColumnData = [&](auto listSpan, BufferPtr& indices) {
          return std::visit(
                ...
                if constexpr(std::is_same_v<T, int8_t> || std::is_same_v<T, int16_t> ||
			     ...
			     std::is_same_v<T, int8_t const> || std::is_same_v<T, int16_t const> ||
        ...
        if(!partialTableIndexColumnName.empty() && columnName == partialTableIndexColumnName) {
	  std::vector<VectorPtr> colDataVec;

	  ExpressionSpanArguments originalListSpans;
	  for(auto& indexBuffer : indicesVec) {
	    auto* indicesData = indexBuffer->as<int32_t>();
	    auto indicesSize = indexBuffer->size() / sizeof(int32_t);
    
	    std::vector<int32_t> indicesVector(indicesData, indicesData + indicesSize);
	    auto* ptr = indicesVector.data();
	    auto size = indicesVector.size();
	    spanReferenceCounter.add(ptr, [v = std::move(indicesVector)]() {});
	    originalListSpans.emplace_back(boss::Span<int32_t>(ptr, size, [ptr]() { spanReferenceCounter.remove(ptr); }));
	  }
  
	  for(auto&& listSpan : originalListSpans) {
	    BufferPtr noIndices = nullptr;
	    colDataVec.emplace_back(getVeloxColumnData(std::move(listSpan), noIndices));
	  }
  
	  colDataListVec.emplace_back(std::move(colDataVec));
	  colNameVec.emplace_back(columnName);
	  colTypeVec.emplace_back(INTEGER());
	  ...
	  return;
        ...
        if(isRadixKeyColumn || otherColumnsSpanIndexes.empty()) {
          ...
          for(auto&& listSpan : listSpans) {
            ...
            colDataVec.emplace_back(getVeloxColumnData(std::move(listSpan), indices));
	  }
        } else {
          ...
          for(auto spanIndex : otherColumnsSpanIndexes) {
            ...
            auto spanRef = std::visit(
                ...
                *sharedSpan);
            colDataVec.emplace_back(getVeloxColumnData(std::move(spanRef), indices));
	  }
        }
...
PlanBuilder Engine::buildOperatorPipeline(
  ...
  if(e.getHead().getName() == "Table" || e.getHead().getName() == "Gather" ||
     ...
     e.getHead().getName() == "PartialTable") {
...
boss::Expression Engine::evaluate(boss::ComplexExpression&& e) {
  ...
  if(e.getHead().getName() == "EvaluateIf" || (e.getHead().getName() != "Group" && e.getHead().getName() != "Order" && e.getHead().getName() != "Top" &&
  ...
  if((e.getHead().getName() == "Group" || e.getHead().getName() == "Order" || e.getHead().getName() == "Top") && maxThreads < 2) {
    maxThreads = 2;
  }
