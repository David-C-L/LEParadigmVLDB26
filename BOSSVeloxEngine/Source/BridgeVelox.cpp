...
TypePtr importFromBossType(BossType bossType) {
  ...
  case boss::engines::velox::bCHAR:
    return TINYINT();
  case boss::engines::velox::bSHORT:
    return SMALLINT();
...
VectorPtr importFromBossAsOwner(BossType bossType, BossArray&& bossArray,
				memory::MemoryPool* pool, BufferPtr indices) {
  ...
  if (indices == nullptr) {
    ...
    return VELOX_DYNAMIC_SCALAR_TYPE_DISPATCH(createFlatVector, type->kind(), pool, type, nullptr,
                                              bossArray.length, values);
  }

  auto* indicesData = indices->as<int32_t>();
  auto indicesSize = indices->size() / sizeof(int32_t);
  
  auto elementSize = type->cppSizeInBytes();
  auto compactedBuffer = AlignedBuffer::allocate<uint8_t>(indicesSize * elementSize, pool);
  auto* compactedData = compactedBuffer->asMutable<uint8_t>();
  
  const auto* sourceBuffer = static_cast<const uint8_t*>(bossArray.buffers);
  for (size_t i = 0; i < indicesSize; ++i) {
    std::memcpy(compactedData + (i * elementSize), 
                sourceBuffer + (i * elementSize), 
                elementSize);
  }
  
  return VELOX_DYNAMIC_SCALAR_TYPE_DISPATCH(createFlatVector, type->kind(), pool, type, nullptr,
                                            indicesSize, compactedBuffer);
}
